...
write(*,*) "Version 3.3.9, release date: 2016-Sep-18"
...
write(*,*) "Multiwfn official website: http://Multiwfn.codeplex.com"
...
write(*,*) "5 Output and plot specific property within a spatial region"
...
if (infuncsel1==0) then !
    ...
    if ((ifiletype==1.or.ifiletype==9).and.imodwfn==0) then !fch and occupation number hasn't been modified
        if (wfntype==0) then
...
!         write(*,*) "test ortho",v1x*v2x+v1y*v2y+v1z*v2z
    ...
    write(*,*)
    write(*,"(' Origin of the plane:',3f10.5,' Bohr')") orgx2D,orgy2D,orgz2D
    ...
    write(*,"(' End of the plane:   ',3f10.5,' Bohr')") endx2D,endy2D,endz2D
...
!$OMP END PARALLEL DO
    509    if (ncustommap/=0) then !cycling will stop when all the file have been dealed
        ...
        do while(.true.)
            write(*,*)
            if (isys==1.or.(nx==ny.and.ny==nz.and.nz==nx)) write(*,*) "-1 Show isosurface graph" !For linux version, this option is present only when nx=ny=nz
            write(*,*) "0 Return to main menu"
...
!10!!------------------- Plot DOS
    ...
    if (ifiletype/=0.or.ifiletype/=1.or.ifiletype/=9) then
        ...
        write(*,"(a,/)") " Error: This function only available for .fch, .molden and plain text file with energy levels!"
    end if
...
!100!!------------------- Misc and some not important functions, Part 1
    ...
    do while(.true.)
        ...
        write(*,*) "2 Output .pdb, .xyz, .wfn, .molden, .fch or Gaussian/GAMESS-US input file"
        ...
        write(*,*) "11 Calculate overlap between norm of two orbitals over the whole space"
...
!         write(*,*) "16 Calculate intermolecular tranfer integral by direct coupling method"
        ...
        if (infuncsel2==0) then
            ...
            if (itmp==1) then
                ...
                if (.not.allocated(shtype)) then
                    write(*,*) "Error: This function works only when .fch/.molden was used as input file"
                else
            ...
            if (ispecial/=1) then
                ...
                call intfunc(ifunc,0,0) !if ifunc/=101, namely integrate two orbital, then the two number is arbitrary
            ...
            write(*,*) "Input the index of the two orbitals, e.g. 34,35"
            read(*,*) iorb1,iorb2
            call intfunc(101,iorb1,iorb2)
        else if (infuncsel2==13) then
            call HOMA_Bird
        else if (infuncsel2==14) then
            call LOLIPOP
