        ...
        if (isetmode==1) then
            ...
            do iGTF=ind1,ind2
                if (seltmpc=="ALL".or.seltmpc=="all") then
                    if (b(iGTF)%center>=iatm1.and.b(iGTF)%center<=iatm2) CO(imo1:imo2,iGTF)=coval
                else
                    if (b(iGTF)%center>=iatm1.and.b(iGTF)%center<=iatm2.and.b(iGTF)%functype==itypfil) CO(imo1:imo2,iGTF)=coval
                end if
            end do
            ...
            write(*,*) "Input the range of atoms"
            read(*,*) iatm1,iatm2
            write(*,"(a)") "Input the type of basis functions (one of S,X,Y,Z,XX,XY... ALL means all types)"
            read(*,*) seltmpc
            write(*,*) "Input the range of orbitals"
            read(*,*) imo1,imo2
            ispinsel=1
            ...
            do ibas=ind1,ind2
                if (ispinsel==0.or.ispinsel==1) then
                    if (seltmpc=="ALL".or.seltmpc=="all") then
                        if (bascen(ibas)>=iatm1.and.bascen(ibas)<=iatm2) CObasa(ibas,imo1:imo2)=coval
                    else
                        if (bascen(ibas)>=iatm1.and.bascen(ibas)<=iatm2.and.bastype(ibas)==itypfil) CObasa(ibas,imo1:imo2)=coval
                    end if
                end if
                if (ispinsel==0.or.ispinsel==2) then
                    if (seltmpc=="ALL".or.seltmpc=="all") then
                        if (bascen(ibas)>=iatm1.and.bascen(ibas)<=iatm2) CObasb(ibas,imo1:imo2)=coval
                    else
                        if (bascen(ibas)>=iatm1.and.bascen(ibas)<=iatm2.and.bastype(ibas)==itypfil) CObasb(ibas,imo1:imo2)=coval
                    end if
                end if
            end do
            imodwfn=1
