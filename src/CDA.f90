...
! Close-shell, unrestricted open-shell, natural orbitals are supported. Restricted open-shell is not supported
...
fragfilename=" "
! fragfilename(1)="x\cda\cobh3\co.out"
! fragfilename(2)="x\cda\cobh3\bh3.out"
...
do ifrag=0,nCDAfrag !Here we first gather basic informations of complex(ifrag=0) and fragments(ifrag>0)
    if (ifrag==0) then
        ...
        if (ifchmol==1) write(*,"(/,a,i4)") " Input Gaussian .fch file of fragment",ifrag
...
cobasCDA=0D0
iopshCDA=0
!If any fragment is open-shell, the CDA will be seen as open-shell, and the close-shell fragment will activate its beta part, namely copy information from alpha part
...
!         if (nmo==2*nbasis) nmo=nbasis !The nmo used in present module is the number of either alpha or beta MOs rather than their sum as in other modules
    ...
    if (iopsh(ifrag)==0) then !Close-shell complex or fragment
        if (ifchmol==0) then
        ...
        if (iopshCDA==1) then !The whole CDA calculation is open-shell, so we duplicate this close-shell fragment as alpha and beta part
...
! ...
!Alpha or total part
allocate(tmpmat(nbasis,nbasis),FOovlpmat(nbasis,nbasis),ovlpbasmatblk(nbasis,nbasis))
...
FOovlpmat=matmul(transpose(tmpmat),matmul(ovlpbasmatblk,tmpmat)) !The resultant FOovlpmat is off-diagonal block matrix to describe FO integrals between different fragments
forall (i=1:nbasis) FOovlpmat(i,i)=1 !All FO integrals within each fragment are zero, here set them to ideal value
! call showmatgau(tmpmat,"tmpmat")
! call showmatgau(FOovlpmat,"FOovlpmat")
!Generate coefficient matrix of complex orbitals in fragment orbital basis
allocate(coFO(nbasis,nmo))
call invmatsub(tmpmat,nbasis)
coFO=matmul(tmpmat,cobasCDA(:,:,0))
deallocate(tmpmat)
!Beta part
if (iopshCDA==1) then
    ...
    FOovlpmatb=matmul(transpose(tmpmat),matmul(ovlpbasmatblk,tmpmat))
    ...
    call invmatsub(tmpmat,nbasis)
    coFOb=matmul(tmpmat,cobasCDAb(:,:,0))
    deallocate(tmpmat)
end if
deallocate(ovlpbasmatblk)

!=== Calculate complex orbital composition in FO basis by Mulliken method
allocate(FOcomp(nmo,nmo))
do imo=1,nmo
    do iFO=1,nmo
        FOcomp(iFO,imo)=coFO(iFO,imo)**2 !Local term
        do jFO=1,nmo
            if (jFO==iFO) cycle
            FOcomp(iFO,imo)=FOcomp(iFO,imo)+coFO(iFO,imo)*coFO(jFO,imo)*FOovlpmat(iFO,jFO)
        end do
    end do
!     write(*,*) imo,sum(FOcomp(:,imo)) !This can be used to validate coFO and FOovlpmat, if both of them are correct, all printed terms should be 1
end do
if (iopshCDA==1) then !Beta part
    allocate(FOcompb(nmo,nmo))
    do imo=1,nmo
        do iFO=1,nmo
            FOcompb(iFO,imo)=coFOb(iFO,imo)**2 !Local term
            do jFO=1,nmo
                if (jFO==iFO) cycle
                FOcompb(iFO,imo)=FOcompb(iFO,imo)+coFOb(iFO,imo)*coFOb(jFO,imo)*FOovlpmatb(iFO,jFO)
            end do
        end do
    end do
end if
...
10        if (iopshCDA==0) refocc=2D0 !Reference orbital occupation number
        ...
        do while(.true.)
            ...
            if (isel2==0) then
                ...
                eneCDAb(:,0)+eneshiftcomp,eneCDAb(:,ifrag)+eneshiftA,eneCDAb(:,jfrag)+eneshiftB,eneplotlow,eneintv,eneplothigh,conncritleft,conncritright,&
                idrawMObar,iconnlogi,ilabelorbidx,ilabelcomp,labsize,complabshift,degencrit,eneshiftA,eneshiftB,eneshiftcomp)
