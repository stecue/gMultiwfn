...
!multiple.txt can records input file of multiple systems, but all systems must have the same number of transitions (i.e. identical numdata)
...
character c200tmp*200,selectyn
...
if (index(filename,"multiple.txt")/=0) then !Multiple file list with weights is recorded in multiple.txt
    ...
    allocate(weight(nsystem))
    rewind(11)
    do i=1,nsystem
        read(11,"(a)") c200tmp
        read(c200tmp,*) c200tmp,weight(i)
        ...
        call loadtransdata(ispectrum,trim(c200tmp),numdata)
        if (.not.allocated(dataxall)) allocate(dataxall(nsystem,numdata),strall(nsystem,numdata),FWHMall(nsystem,numdata))
        dataxall(i,:)=datax
        strall(i,:)=str
        FWHMall(i,:)=FWHM
    end do
    ...
    allocate(dataxall(nsystem,numdata),strall(nsystem,numdata),FWHMall(nsystem,numdata))
    dataxall(1,:)=datax
    strall(1,:)=str
    FWHMall(1,:)=FWHM
end if
...
do while(.true.)
    ...
    if (maxval(FWHMall)==minval(FWHMall)) then
        if (iunitx==0) then
    ...
    if (nsystem>1.and.iweisyscurve==0) write(*,*) "18 Toggle weighting spectrum of each system, current: OFF"
    if (nsystem>1.and.iweisyscurve==1) write(*,*) "18 Toggle weighting spectrum of each system, current: ON"
    ...
    if (isel==0.or.isel==1.or.isel==2.or.isel==15.or.isel==16) then
        ...
        if (iusersetx==0) then !Automatical scale, if user has not manually set the range
            if (iunitx==0) then !cm^-1 for IR, Raman, VCD
                ...
                xhigh=1240.7011D0/ (1240.7011D0/minval(dataxall)+40) !Note that when nm is used, in common spectrum the energy is from high to low, so we invert xlow and xhigh
                xlow=1240.7011D0/ (1240.7011D0/maxval(dataxall)-40)
            else
                rangetmp=maxval(dataxall)-minval(dataxall)
                if (rangetmp==0) rangetmp=abs(dataxall(1,1))
                xlow=minval(dataxall)-0.3D0*rangetmp
                xhigh=maxval(dataxall)+0.3D0*rangetmp
            end if
    ...
    if (isel==2.or.isel==15) then !Output curve for total and individual contributions, respectively
        ...
        do imol=1,nsystem
            do i=1,3*numdata
                write(10,"(2f16.5)") linexall(imol,i),lineyall(imol,i)
            end do
            write(10,*)
        end do
        close(10)
        ...
        if (nsystem>1) write(*,*) "Note: The height of discrete lines in this file have been weighted"
        if (ispectrum==1) then !IR
...
!!----- Load transition data from a file
