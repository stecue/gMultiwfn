...
real*8 x,y,z
userfunc=1.0D0 !Default value. Note: default "iuserfunc" is 0
...
if (idir==0) then
    lagkin=0D0
    do i=1,nmo
        lagkin=lagkin+MOocc(i)*sum(wfnderv(:,i)**2)
    end do
else
    lagkin=sum(MOocc(:)*wfnderv(idir,:)**2)
end if
lagkin=lagkin/2D0
end function
...
!refx,refy,refz is reference point, namely r1, stored in defvar module
...
real*8 function pairfunc(x,y,z)
real*8 x,y,z,orbvalr1(nmo),orbvalr2(nmo)
call orbderv(1,1,nmo,refx,refy,refz,orbvalr1)
call orbderv(1,1,nmo,x,y,z,orbvalr2)
...
if (pairfunctype==12) then
    pairfunc=adensr1*(adensr2+bdensr2)+aXCdens +bdensr1*(adensr2+bdensr2)+bXCdens
end if
end function
...
!!!------------------ Product of norm of two orbitals
real*8 function twoorbnorm(x,y,z,inp1,inp2)
real*8 x,y,z
integer inp1,inp2
twoorbnorm=abs(fmo(x,y,z,inp1))*abs(fmo(x,y,z,inp2))
end function
...
!!!-------------- Single exponential decay detector (SEDD), see DOI: ChemPhysChem, 13, 3462 (2012)
!Hasn't been available in Multiwfn, the results are unlike to original paper, I don't know why...
real*8 function SEDD(x,y,z)
real*8 x,y,z
rdens=fdens(x,y,z)
rder2=fgrad(x,y,z,'t')**2
rlapl=flapl(x,y,z,'t')
epsilon=rder2*( 2*rlapl/rdens**3-2*rder2/rdens**4 )**2
SEDD=dlog(epsilon)
end function
...
derv2y=(derv1add(2)-derv1min(2))/(2*diffstep) !d2v/dx2
